// Exact exhaustive verifier for the 6x12 -> 4x8 -> 2x6 certificate.
#include <algorithm>
#include <fstream>
#include <iostream>
#include <limits>
...
Certificate ReadCertificate(const std::string& path) {
  ...
  for (const auto& [width, height] :
       {std::pair{6, 12}, std::pair{4, 10}, std::pair{2, 8}}) {
...
int main(int argc, char** argv) {
  try {
    const Certificate certificate =
        ReadCertificate(argc == 2 ? argv[1] : "pyramid_6x12.cert");
    long long total = 0;
    ...
    std::cout << "{\"initial_slices\":4722366482869645213696,"
