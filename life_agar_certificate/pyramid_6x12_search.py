#!/usr/bin/env python3
"""Column-generate the 6x12 shrinking-pyramid upper certificate."""
...
def write_certificate(path, weights, maximum):
    ...
    lines = [
        "# Generated by pyramid_6x12_search.py.",
    ...
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return Fraction(int(maximum * scale), total)
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).with_name("pyramid_6x12.cert"),
    )
    args = parser.parse_args()
    weights, maximum = solve()
    bound = write_certificate(args.output, weights, maximum)
    print(f"wrote {args.output}: verified bound {bound}")
